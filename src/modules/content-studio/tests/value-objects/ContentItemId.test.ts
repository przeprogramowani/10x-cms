import {describe, it, expect} from "vitest";
import {ContentItemId} from "../../domain/value-objects/ContentItemId";

describe("ContentItemId", () => {
  describe("generate", () => {
    it("should generate a valid ContentItemId", () => {
      const id = ContentItemId.generate();
      expect(id).toBeInstanceOf(ContentItemId);
      expect(id.toString()).toBeDefined();
      expect(typeof id.toString()).toBe("string");
    });

    it("should generate unique IDs", () => {
      const id1 = ContentItemId.generate();
      const id2 = ContentItemId.generate();
      expect(id1.toString()).not.toBe(id2.toString());
    });

    it("should generate valid UUID format", () => {
      const id = ContentItemId.generate();
      const uuidRegex =
        /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
      expect(id.toString()).toMatch(uuidRegex);
    });
  });

  describe("fromString", () => {
    it("should create ContentItemId from valid string", () => {
      const value = "test-id-123";
      const id = ContentItemId.fromString(value);
      expect(id.toString()).toBe(value);
    });

    it("should throw error for empty string", () => {
      expect(() => ContentItemId.fromString("")).toThrow(
        "ContentItemId must be a non-empty string"
      );
    });

    it("should throw error for whitespace-only string", () => {
      expect(() => ContentItemId.fromString("   ")).toThrow(
        "ContentItemId must be a non-empty string"
      );
    });

    it("should throw error for null", () => {
      expect(() => ContentItemId.fromString(null as any)).toThrow(
        "ContentItemId must be a non-empty string"
      );
    });

    it("should throw error for undefined", () => {
      expect(() => ContentItemId.fromString(undefined as any)).toThrow(
        "ContentItemId must be a non-empty string"
      );
    });
  });

  describe("equals", () => {
    it("should return true for same ID values", () => {
      const value = "test-id-123";
      const id1 = ContentItemId.fromString(value);
      const id2 = ContentItemId.fromString(value);
      expect(id1.equals(id2)).toBe(true);
    });

    it("should return false for different ID values", () => {
      const id1 = ContentItemId.fromString("id-1");
      const id2 = ContentItemId.fromString("id-2");
      expect(id1.equals(id2)).toBe(false);
    });

    it("should return false for null", () => {
      const id = ContentItemId.fromString("test-id");
      expect(id.equals(null as any)).toBe(false);
    });

    it("should return false for undefined", () => {
      const id = ContentItemId.fromString("test-id");
      expect(id.equals(undefined as any)).toBe(false);
    });

    it("should be reflexive - id equals itself", () => {
      const id = ContentItemId.generate();
      expect(id.equals(id)).toBe(true);
    });

    it("should be symmetric - if id1 equals id2, then id2 equals id1", () => {
      const value = "test-id-123";
      const id1 = ContentItemId.fromString(value);
      const id2 = ContentItemId.fromString(value);
      expect(id1.equals(id2)).toBe(id2.equals(id1));
    });
  });

  describe("toString", () => {
    it("should return string representation of ID", () => {
      const value = "test-id-123";
      const id = ContentItemId.fromString(value);
      expect(id.toString()).toBe(value);
    });

    it("should preserve the original value", () => {
      const value = "my-custom-id-with-dashes";
      const id = ContentItemId.fromString(value);
      expect(id.toString()).toBe(value);
    });
  });
});
